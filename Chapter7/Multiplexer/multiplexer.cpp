...
class MultiplexServer{
    ...
    private:
        ...
        void onBackendConnection(const TcpConnectionPtr& conn){
            ...
            std::vector<TcpConnectionPtr> connsToDestory;
            if(conn->connected()){
                MutexLockGuard lock(mutex_);
                backendConn_ = conn;
                assert(availIds_.empty());
                ...
                MutexLockGuard lock(mutex_);
                backendConn_.reset();
                connsToDestory.reserve(clientConns_.size());
                for(std::map<int, TcpConnectionPtr>::iterator it = clientConns_.begin();
                    it != clientConns_.end();
                    ++it){
                    connsToDestory.push_back(it->second);
                }
                clientConns_.clear();
            ...
            for(std::vector<TcpConnectionPtr>::iterator it = connsToDestory.begin();
                it != connsToDestory.end();
                ++it){
                (*it)->shutdown();
            }
        }
