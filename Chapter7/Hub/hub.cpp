        ...
        private:
            ...
            void onMessage(const TcpConnectionPtr& conn,
                ...
                while(result == kSuccess){
                    ...
                    string topic;
                    string content;
                    result = parseMessage(buffer, &cmd, &topic, &content);
                    if(result == kSuccess){
                        if(cmd == "pub"){
            ...
            void timePublish(){
                ...
                doPublish("inetrnal", "utc_time", now.toFormattedString(), now);
            }
            ...
            void doUnsubscribe(const TcpConnectionPtr& conn,
                ...
                // topic could be the one to be destoryed, so don't use it after erasing.
                ConnectionSubscription* connSub
                    = boost::any_cast<ConnectionSubscription>(conn->getMutableContext());
                connSub->erase(topic);
            }
